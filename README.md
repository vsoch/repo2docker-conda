...
This is a continuous build template based on the [python_conda_pip](https://github.com/binder-examples/python_conda_pip) binder example, meaning that it shows how you can install Miniconda alongside Python (meaning Jupyter
notebooks!) and then deploy a container automatically using Docker Hub.

 - the environment.yml file is for the MiniConda Python environment
...
The presence of an `environment.yml` file will tell the repo2docker to build a container
with a Miniconda installation! You can use this example as a template to create a complete
build->test->deploy pipeline on CircleCI.

If you intend to use pip, don't use a `requirements.txt` - add a `- pip` section in
...
This template will let you do the following:
 ...
 1. Add your notebook (e.g., like the [index.ipynb](index.ipynb) file) and dependencies (environment.yml) to a Github repository
...
First, fork the [repo2docker-conda](https://www.github.com/vsoch/repo2docker-conda/)
Github repository to your account, and clone the branch.

```bash
git clone https://www.github.com/<username>/repo2docker-conda
git clone git@github.com:<username>/repo2docker-conda.git
```
...
along with your dependencies files.
...
```bash
docker pull vanessa/repo2docker-conda
docker run -it --name repo2docker -p 8888:8888 vanessa/repo2docker-conda jupyter notebook --ip 0.0.0.0
```
...
## Support
If you want to get help please [post an issue!](https://www.github.com/vsoch/repo2docker-conda/issues)
